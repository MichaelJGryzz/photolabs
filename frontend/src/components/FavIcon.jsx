...
const FavIcon = ({ displayAlert, selected, openFavedPhotosModal }) => {
  return (
    <svg
      width="20"
      height="17"
      ...
      onClick={displayAlert ? openFavedPhotosModal : null} // Handle click only if alert is displayed
      style={{ cursor: displayAlert ? 'pointer' : 'default' }} // Change cursor when clickable
    >
      <path
      ...
      {
        !!displayAlert &&
        <circle cx="21" cy="4" r="3.5" fill="#40FF00" stroke="#C80000" strokeWidth="0.5"/>
      }
    </svg>
  );
};
